...
if(isPvp){
  ...
  $('.dialog-eyebrow').textContent='СЕТЕВАЯ ДУЭЛЬ';
  $('#setup-dialog h2').textContent='Выберите хранителя';
  $('#setup-dialog .dialog-description').innerHTML='Создайте комнату или введите код друга.<br>Бой начнётся, когда подключатся оба игрока.';
  $('#enemy-select').closest('div').hidden=true;
  $('#swap').hidden=true;
  $('#start').hidden=true;
  $('#setup-atlas').insertAdjacentHTML('beforebegin',`
...
async function leavePvp(message='Комната закрыта.',notify=true){
  ...
  $('#pvp-room-status').hidden=true;$('#create-room').hidden=false;$('.pvp-divider').hidden=false;$('#room-code').parentElement.previousElementSibling.hidden=false;$('#room-code').parentElement.hidden=false;$('#player-select').disabled=false;$('#setup-dialog').showModal();
...
function configure(){clearTimeout(resultTimer);closeDialogs();battle=new Battle({player:playerHero,enemy:enemyHero,ai:!isPvp});renderer?.reset();updateSelection();$('#setup-dialog').showModal();updateHUD();}
...
$('#swap').onclick=swap;$('#start').onclick=start;$('#rematch').onclick=()=>isPvp?void leavePvp('Создайте новую комнату для реванша.'):start();
$('#result-swap').onclick=()=>{if(isPvp)void leavePvp('Выберите хранителя для новой комнаты.');else{swap();configure();}};$('#new-match').onclick=configure;
