...
<main class="game-shell">
  <section class="combatant enemy-panel" aria-label="Противник"><img id="enemy-portrait" alt=""><div class="combatant-info"><div class="name-row"><span id="enemy-name">Керемль</span><small>СОПЕРНИК</small></div><div class="hp-track"><div id="enemy-bar" class="hp-fill enemy-fill"></div><div id="enemy-shield" class="shield-fill"></div></div><div class="health-meta"><span id="enemy-hp"></span><span id="enemy-status"></span></div></div><div class="timer"><strong id="timer">90</strong><small>СЕК</small></div><button class="icon-button" id="menu" aria-label="Пауза и меню">Ⅱ</button></section>
  <section class="arena" id="arena"><div class="load-panel" id="loading"><span class="loading-emblem">Ⅲ</span><p id="loading-message">Пробуждаем хранителей…</p><progress id="load-progress" max="1" value="0"></progress></div><div class="toast" id="toast" role="status"></div></section>
...
<dialog id="pause-dialog" class="panel-dialog"><div class="dialog-eyebrow">ВРЕМЯ ОСТАНОВЛЕНО</div><h2>Пауза</h2><p id="pause-reason" class="dialog-description">Можно перевести дух.</p><button class="primary-button" id="resume">Продолжить бой</button><button class="text-button" id="pause-rules">Как играть</button><button class="text-button" id="sound" aria-label="Включить звук" title="Звук выключен">♪ Звук выключен</button></dialog>
...
$('#pause-rules').onclick=()=>openSub('#rules-dialog');$('#close-rules').onclick=()=>closeSub('#rules-dialog');
...
$('#sound').onclick=async()=>{soundOn=!soundOn;$('#sound').classList.toggle('enabled',soundOn);$('#sound').setAttribute('aria-label',soundOn?'Выключить звук':'Включить звук');$('#sound').title=soundOn?'Звук включён':'Звук выключен';$('#sound').textContent=soundOn?'♪ Звук включён':'♪ Звук выключен';if(soundOn){const Audio=window.AudioContext||window.webkitAudioContext;if(Audio){audio??=new Audio();await audio.resume();playSound('CAST');}}};
...
try{
  ...
  await renderer.init(progress=>{$('#load-progress').value=progress;},[{hero:playerHero,face:'back'},{hero:enemyHero,face:'front'}]);loaded=true;$('#loading').hidden=true;
