...
export function executeDemo(
  ...
  if (command.type === 'capture') {
    ...
    if (Date.parse(progress.cooldowns[command.characterId] || '') > now)
    ...
    if (gradeQuiz(command.characterId, command.answers)) {
    ...
    progress.cooldowns[command.characterId] = new Date(
      now + COOLDOWN_MS,
    ).toISOString()
